...
{
	if (othread->page_table != nthread->page_table) 
	{
		__asm__ volatile ("mov cr3, %0" :: "b"(nthread->page_table));
		__asm__ volatile ("mov cr0, %0" :: "b"(get_cr0() | 0x80000000));
	}
	ctxt_sw((void**)&othread->stack_pointer, (void*)nthread->stack_pointer);
}
